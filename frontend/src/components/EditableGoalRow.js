...
export default function EditableGoalRow({ goal, setError, exercises }) {
  ...
  const handleSubmit = (e) => {
    ...
    const fetchData = async () => {
      ...
      const response = await fetch(`https://backend-l1of.onrender.com/api/goals/${goal._id}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleClickDelete = () => {
    fetch(`https://backend-l1of.onrender.com/api/goals/${goal._id}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${user.token}`,
      },
  ...
  const findRemaining = useCallback(
    (exercise, goalType) => {
